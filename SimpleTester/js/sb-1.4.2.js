...
Spacebrew.Client = function( server, name, description, options ){
	...
	/**
 	* Secure 1==true 0==false
 	* @type {Integer}
 	*/
	this.secure = options.secure || 0;
	if (window) {
		var secure = window.getQueryString("secure");
		if (secure == 1) {
			this.secure = secure;
		}
	}
...
Spacebrew.Client.prototype.connect = function(){
	try {
		...
		console.log("ws connecting with:" + protocol);
		this.socket = new WebSocket(protocol + this.server + ":" + this.port);
